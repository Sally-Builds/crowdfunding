...
class ProjectController implements Controller {
    ...
    private initializeRouter(): void {
        this.router
            .route(`${this.path}`)
            .post(authenticated, role('admin'), validationMiddleware(validate.create), this.createProject)
            .get(this.getAllProjects)

        this.router
            .route(`${this.path}/:id`)
            .get(this.getProject)
            .patch(authenticated, role('admin'), validationMiddleware(validate.update), this.updateProject)
    }
    ...
    private getProject = async (req: Request, res: Response, next:NextFunction): Promise<Response | void> => {
        try {
            const {id} = req.params
    ...
    private updateProject = async(req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
        try {
            const {id} = req.params
            const {name, expectedAmount, consensusStatus} = req.body
            const project = await this.ProjectService.update(id, name, expectedAmount, consensusStatus)
            res.status(200).json({
                status: 'success',
                project,
            })
